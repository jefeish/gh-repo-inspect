# Usage Examples for gh repo-inspect

This document provides practical examples of using the `gh repo-inspect` extension.
...
```bash
gh extension install jefeish/gh-repo-inspect
```
...
gh repo-inspect microsoft/vscode
```
...
```bash
# JSON output (default)
gh repo-inspect microsoft/vscode --format json

# YAML output
gh repo-inspect microsoft/vscode --format yaml

# Human-readable table
gh repo-inspect microsoft/vscode --format table
```
...
gh repo-inspect microsoft/vscode --sections branches

# Multiple sections
gh repo-inspect microsoft/vscode --sections branches,security,collaborators
...
for repo in "${repos[@]}"; do
    echo "Auditing $repo..."
    gh repo-inspect "$repo" --format json > "${repo//\//_}-audit.json"
...
gh repo-inspect myorg/myrepo --sections security,branches --format table

# Export detailed governance configuration
gh repo-inspect myorg/myrepo --format yaml > governance-config.yaml
```
...
gh repo-inspect myorg/myrepo --verbose
```
...
```bash
gh repo-inspect myorg/repo1 --sections security --format table
gh repo-inspect myorg/repo2 --sections security --format table
```
...
```bash
gh repo-inspect myorg/critical-repo --sections branches --verbose
```
...
```bash
gh repo-inspect myorg/private-repo --sections collaborators,teams --format json
```
...
gh repo-inspect myorg/repo --format yaml > compliance-report.yaml

# Create summary for multiple repos
for repo in repo1 repo2 repo3; do
    gh repo-inspect "myorg/$repo" --format json > "${repo}-compliance.json"
done
```
...
```bash
gh repo-inspect myorg/repo --verbose --sections security
```